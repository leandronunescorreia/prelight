                                     ...
                                     WCHAR* strExePath, WCHAR* strExeName )
{
	...
	// $ NV BD Add in the NV SDK Search path
	// Search in ..\\..\\..\\..\\MEDIA  
	StringCchPrintf( strSearchPath, cchSearch, L"..\\..\\..\\..\\MEDIA\\%s", strLeaf );
	if( GetFileAttributes( strSearchPath ) != 0xFFFFFFFF )
		return true;

	// $$ This extra one is needed since the DX sample framewok here loads some resources, 
	//	and I'm trying to keep my changes to a minimum
	// Search in ..\\..\\..\\..\\MEDIA\\textures 
	StringCchPrintf( strSearchPath, cchSearch, L"..\\..\\..\\..\\MEDIA\\textures\\%s", strLeaf );
	if( GetFileAttributes( strSearchPath ) != 0xFFFFFFFF )
		return true;

	// Incase of extra directory level
	StringCchPrintf( strSearchPath, cchSearch, L"..\\..\\..\\..\\..\\MEDIA\\%s", strLeaf );
	if( GetFileAttributes( strSearchPath ) != 0xFFFFFFFF )
		return true;
...
VOID DXUTLaunchReadme( HWND hWnd, LPCWSTR strLoc )
{
    ...
    for( int i=0; i < NUM_FILENAMES; i++ )
    {
        ...
        if( SUCCEEDED(hr) )
        {
            ...
            HINSTANCE hInstShell32 = LoadLibrary( wszPath );
...
HRESULT CDXUTDirectionWidget::StaticOnCreateDevice( IDirect3DDevice9* pd3dDevice )
{
    ...
    V_RETURN( DXUTFindDXSDKMediaFileCch( str, MAX_PATH, L"Resources\\DXUTShared.fx" ) );
    ...
    V_RETURN( DXUTFindDXSDKMediaFileCch( str, MAX_PATH, L"Resources\\arrow.x" ) );
