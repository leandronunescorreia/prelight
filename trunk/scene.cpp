...
void Scene::Reset()
{
    ...
    mMeshes.clear();
    mMaterials.clear();
    for (int i = 0, len = mTextures.size(); i < len; i++)
        SAFE_RELEASE(mTextures[i]);
    mTextures.clear();
}

Model::Model(const std::string& name, const mat4& worldMatrix, const Mesh* mesh, const Material* material)
: mName(name)
...
Material::Material(const std::string& name, const vec4& diffuse, const vec4& specular, const vec4& shininess)
: mName(name)
, mDiffuse(diffuse)
, mSpecular(specular)
...
HRESULT Scene::Load(const tstring& filename, LPDIRECT3DDEVICE9 device, GetFilePathCallback getFilePath)
{
	Free();
	m_Device = device;
...
HRESULT Scene::LoadTexture(const tstring& name, LPDIRECT3DTEXTURE9& texture, GetFilePathCallback getFilePath, D3DFORMAT format, D3DPOOL pool)
{
	HRESULT hr;
	tstring tempName = name;
	if (name == _T(""))
	{
		tempName = _T("ruf-wood.tga");
	}
	tstring fullname = (getFilePath ? (*getFilePath)(tempName) : tempName);
...
HRESULT Scene::LoadTextures(GetFilePathCallback getFilePath)
{
	HRESULT hr;
	ReleaseTextures();
...
HRESULT Scene::LoadMeshes(GetFilePathCallback getFilePath)
{
	HRESULT hr;
...
HRESULT Scene::LoadCameras()
{
	m_Cameras = new Camera [m_NumCameras];
	return S_OK;
}

HRESULT Scene::LoadLights()
{
...
void Scene::Update(float time, const D3DXMATRIX* transform, unsigned int what)
{
	if (m_Scene.nodes == 0)
		return;
...
HRESULT Scene::DrawNormals(const D3DXMATRIX* projection, const D3DXMATRIX* view, const D3DXMATRIX* world)
{
	HRESULT hr;
	if (projection == 0)
		if (m_NumCameras > 0)
...
HRESULT Scene::DrawTangentBasis(const D3DXMATRIX* projection, const D3DXMATRIX* view, const D3DXMATRIX* world)
{
	HRESULT hr;
	if (projection == 0)
		if (m_NumCameras > 0)
...
HRESULT Scene::BaseMesh::Draw() const
{
	HRESULT hr;
...
HRESULT Scene::BaseMeshIndexed::Draw() const
{
	HRESULT hr;
	if (FAILED(hr = m_Device->SetStreamSource(0, m_VertexBuffer, 0, sizeof(Vertex))))
		return hr;
...
HRESULT Scene::Mesh::DrawNormals(const D3DXMATRIX& projection, const D3DXMATRIX& view, const D3DXMATRIX& world) const
{
...
HRESULT Scene::Mesh::DrawTangentBasis(const D3DXMATRIX& projection, const D3DXMATRIX& view, const D3DXMATRIX& world) const
{
...
HRESULT Scene::CreateCoordinateAxis()
{
	HRESULT hr;
...
HRESULT Scene::DrawCoordinateAxis(const D3DXMATRIX* projection, const D3DXMATRIX* view, const D3DXMATRIX* world)
{
	HRESULT hr;
	if (projection == 0)
		if (m_NumCameras > 0)
...
void Scene::MatToD3DXMATRIX(const mat4& mat, D3DXMATRIX& D3DMat)
{
