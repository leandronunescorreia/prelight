...
const tstring PreLightShading::m_DefaultDiffuseMapFilename(_T("basetex.bmp"));
const tstring PreLightShading::m_DefaultNormalMapFilename(_T("wavy_normal.dds"));
const tstring PreLightShading::m_DefaultEnvironmentMapFilename(_T("NV-Metal-b.dds"));
const float	  PreLightShading::m_DefaultAmbient = 0.1f;

#define ID_QUIT              (USER_ID_START_VALUE + 0)


//-----------------------------------------------------------------------------
// Name: PreLightShading()
...
//-----------------------------------------------------------------------------
PreLightShading::PreLightShading() :
m_bLoadingApp(true),
m_bCanTonemap(true),
m_pEffect(NULL),
...
//-----------------------------------------------------------------------------
HRESULT PreLightShading::OneTimeSceneInit()
{
	srand(0);
...
bool PreLightShading::CanTonemap(LPDIRECT3D9 m_pD3D, D3DDEVTYPE devType)
{
	HRESULT hr = D3D_OK;
...
void PreLightShading::InitializeLights()
{
	m_Lights.clear();
...
void PreLightShading::AnimateLights()
{
...
//-----------------------------------------------------------------------------
HRESULT PreLightShading::ConfirmDevice(D3DCAPS9* pCaps, DWORD dwBehavior,
...
HRESULT PreLightShading::CreateQuad( IDirect3DDevice9* pd3dDevice, IDirect3DVertexBuffer9* &quadVB, unsigned int width, unsigned int height)
{
	HRESULT hr = S_OK;
...
//-----------------------------------------------------------------------------
HRESULT PreLightShading::RestoreDeviceObjects(IDirect3DDevice9* pd3dDevice)
{
    HRESULT hr;
...
//-----------------------------------------------------------------------------
HRESULT PreLightShading::Render(IDirect3DDevice9* pd3dDevice)
{
	HRESULT hr = S_OK;
...
HRESULT PreLightShading::BlurSurface(IDirect3DDevice9* pd3dDevice,IDirect3DTexture9* sourceTex)
{
	HRESULT hr;
...
RECT PreLightShading::DetermineClipRect(const D3DXVECTOR3& position, const float range)
{
...
HRESULT PreLightShading::DrawFinalScene(IDirect3DDevice9* pd3dDevice)
{
    HRESULT hr = S_OK;
...
HRESULT PreLightShading::DrawLightTransport(IDirect3DDevice9* pd3dDevice)
{
    HRESULT hr = S_OK;
...
HRESULT PreLightShading::DrawSceneToShadowMap(IDirect3DDevice9* pd3dDevice)
{
	HRESULT hr = S_OK;
	...
	{  
		for (UINT uPass = 0; uPass < uPasses; uPass++) 
		{
            ...
            for (unsigned int i = 0; i < m_Scene->m_NumMeshes; ++i) 
			{
                ...
                if ( (0 == mesh.m_Material.compare("stucko_Outside")) || (0 == mesh.m_Material.compare("stucko_Inside"))
                    || (0 == mesh.m_Material.compare("logs")) || (0 == mesh.m_Material.compare("old_wood")) || (0 == mesh.m_Material.compare("Front_door")))
				{
...
void PreLightShading::AddPointLight(float x, float y, float z)
{
	bool bFound = false;
	for (unsigned int i = 0; i < m_Lights.size(); ++i)
	{
...
HRESULT PreLightShading::DrawSceneToMRT(IDirect3DDevice9* pd3dDevice)
{
    HRESULT hr = S_OK;
...
bool PreLightShading::IsShadowedMesh(const std::string meshName)
{
	if (m_bSupportsHWShadowMaps)
...
void PreLightShading::SetBumpDot3VSConstantsSM(const D3DXMATRIX& world, const D3DXMATRIX& viewProj)
{
    D3DXMATRIX worldViewProj = world * viewProj;
    m_pEffect->SetMatrix("WorldViewProj", &worldViewProj);
}

void PreLightShading::SetBumpDot3VSConstants(const D3DXMATRIX& world)
{
...
//-----------------------------------------------------------------------------
HRESULT PreLightShading::InvalidateDeviceObjects()
{
	ReleaseTextures();
...
HRESULT PreLightShading::LoadShaders(IDirect3DDevice9* pd3dDevice)
{
    HRESULT hr = D3D_OK;

    // Load our main Effect file, which contains our shaders.  (note: path is relative to MEDIA\ dir)
    hr = D3DXCreateEffectFromFile(pd3dDevice, GetFilePath::GetFilePath(_T("programs\\PreLightShading\\PreLightShading.fx")).c_str(),
...
//-----------------------------------------------------------------------------
HRESULT PreLightShading::FinalCleanup()
{
    return S_OK;
}

HRESULT PreLightShading::InitializeScene(IDirect3DDevice9* pd3dDevice)
{
	HRESULT hr;

	// Scene
	m_Scene = new Scene;
	if (FAILED(hr = m_Scene->Load(m_SceneFilename, pd3dDevice, GetFilePath::GetFilePath))) {
		TCHAR buf[1024];
...
HRESULT PreLightShading::LoadTexture(IDirect3DDevice9* pd3dDevice, const tstring& filename, LPDIRECT3DTEXTURE9& texture, D3DPOOL pool, D3DXIMAGE_INFO* info)
{
...
HRESULT PreLightShading::LoadTexture(IDirect3DDevice9* pd3dDevice, const tstring& filename, LPDIRECT3DCUBETEXTURE9& texture, D3DXIMAGE_INFO* info)
{
	SAFE_RELEASE(texture);
	HRESULT hr;
...
void PreLightShading::ReleaseTextures()
{
...
HRESULT PreLightShading::LoadTextures(IDirect3DDevice9* pd3dDevice)
{
	ReleaseTextures();
	HRESULT hr;
...
HRESULT PreLightShading::CheckResourceFormatSupport(IDirect3DDevice9* pd3dDevice, D3DFORMAT fmt, D3DRESOURCETYPE resType, DWORD dwUsage)
{
    HRESULT hr = S_OK;
